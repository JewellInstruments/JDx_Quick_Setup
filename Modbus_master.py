...
class ModbusRTU:
    ...
    def _validate_frame(self, frame: bytes) -> bool:
        ...
        try:
            ...
            error = f"ERROR :::: {e.__traceback__.tb_lineno} - {e}"
            print(error)
            received_crc = 0
            # raise
    ...
    def _send_frame(self, frame: bytes, read_size: int = 1000) -> bytes | None:
        ...
        try:
            ...
            # Read response
            response = self.serial.read(read_size).hex(
                " ",
            )  # Read available data
    ...
    def _parse_response(
        ...
        if function_code & 0x80:
            ...
            print(
                f"❌ Modbus exception: {self._get_exception_message(exception_code)}",
            )
            return None
...
class ModbusMaster(ModbusRTU):
    ...
    def write_multiple_coils(
        ...
        data = (
            struct.pack(">HHB", start_address, len(values), byte_count)
            + coil_bytes
        )
